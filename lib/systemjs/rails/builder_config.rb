  ...
  module Rails
    class BuilderConfig
      ...
      def module_name(file_path)
        ...
        system_config_paths.each do |map, regex|
      ...
      def system_config
        @system_config ||= begin
          ...
          JSON.parse(context.call('System.fetchConfig'))
        end
      end

      def system_config_paths
        @system_config_paths ||= begin
          sorted_paths = Hash[system_config['paths'].sort_by { |key, value| value.length }.reverse]
